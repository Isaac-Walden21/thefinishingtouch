...
import {
  Workflow, Play, Pause, Mail, Clock, Users, CheckCircle, ChevronDown, Zap,
} from "lucide-react";
...
export default function MarketingAutomationsPage() {
  ...
  return (
    <div className="p-4 pt-16 lg:p-8 lg:pt-8">
      <PageHeader title="Automations" subtitle="Drip sequences that run automatically based on CRM triggers." />
      ...
      <div className="space-y-6">
        {demoAutomations.map((automation) => {
          ...
          return (
            <div key={automation.id} className="rounded-xl border border-slate-200 bg-surface shadow-sm p-6">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className="rounded-lg p-2.5 bg-purple-50"><Workflow className="h-5 w-5 text-purple-600" /></div>
                  <div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className={`rounded-lg p-2 transition-colors ${currentStatus === "active" ? "bg-emerald-50 text-emerald-600 hover:bg-emerald-100" : "bg-slate-100 text-slate-400 hover:bg-slate-200"}`}
                  >
                    {currentStatus === "active" ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}
                  </button>
                  <button onClick={() => setExpandedId(isExpanded ? null : automation.id)} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-600">
              ...
              {isExpanded && (
                <div className="mt-4 pt-4 border-t border-slate-200">
                  <p className="text-xs font-medium text-slate-500 mb-3 uppercase">Email Sequence</p>
                  <div className="space-y-3">
                    {automation.emails.map((email, i) => (
                      <div key={email.id} className="flex gap-4">
                        <div className="flex flex-col items-center">
                          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-brand/10 text-xs font-bold text-brand">{i + 1}</div>
